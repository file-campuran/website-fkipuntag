...
class Profil extends MY_Controller{
    ...
    function index()
    {
        ...
        $data = array(
            'title'                 => 'profil Baru | '.$site['nama_website'],
            'favicon'               => $site['favicon'],
            'site'                  => $site,
        );
        $this->template->load('alayout/template', 'admin/menu/profil/index', $data);
    }

    function get_data()
	{
        $site = $this->Konfigurasi_model->listing();
		$list = $this->sub_menu_model->get_datatables(1);
        $data = array();
		...
		foreach ($list as $field) {
            ...
            $row[]  = '<a href="'.site_url('admin/menu/profil/edit/'.$field->id_sub_menu).'" class="btn btn-warning btn-sm" title="Sunting Kata"><span class="fa  fa-pencil "></span></a>'.' <a href="'.site_url('admin/menu/profil/hapus/'.$field->id_sub_menu).'" class="btn btn-danger btn-sm" title="Hapus Permanen"  onclick="return confirm('.$kata.')"><span class="fa fa-trash"></span></a>';
			$data[] = $row;
		}
    ...
    {   
        ...
        {  
            ...
            redirect('admin/menu/profil');
        }
        else
        {
            $this->template->load('alayout/template', 'admin/menu/profil/add', $data);
    ...
    {   $site = $this->Konfigurasi_model->listing();
        $data = array(
            'title'                 => 'Edit profil | '.$site['nama_website'],
            'favicon'               => $site['favicon'],
            'site'                  => $site,
        );
        $data['profil'] = $this->sub_menu_model->detail_data($id_sub_menu);
        $this->template->load('alayout/template', 'admin/menu/profil/edit', $data);
    }
    ...
    {   
        ...
        redirect('admin/menu/profil/edit/'.$id_sub_menu);
    ...
    function hapus($id)
    {   
        $profil = $this->sub_menu_model->detail_data($id);
        if(isset($profil->id_sub_menu))
        {
            $this->sub_menu_model->hapus_data($profil->id_sub_menu);
            unlink('./assets/upload/foto_artikel/'.$profil->gambar);
            redirect('admin/menu/profil');
        }
        else
    ...
    function upload_foto($nama){
        ...
        if ( ! $upload )
        {
            ...
            $this->template->load('alayout/template', 'admin/menu/profil/add', $error);
        }
    ...
    {
        $profil = $this->sub_menu_model->detail_data($id_sub_menu);

        if(isset($profil->id_sub_menu))
        {
            unlink('./assets/upload/foto_artikel/'.$profil->gambar);
            $data['gambar'] = "";
            $this->sub_menu_model->update_data($data, $profil->id_sub_menu);
            redirect('admin/menu/profil/edit/'.$id_sub_menu);
        }
        else
